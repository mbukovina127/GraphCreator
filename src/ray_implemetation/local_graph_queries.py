...
class LocalGraphQueries:
    ...
    def _get_node(self, key: str, collection: str = "nodes") -> Optional[Dict[str, Any]]:
        ...
        return self.GRAPH_BUILDER.get_node(collection, key)
    ...
    def _get_children(self, parent_key: str, collection: str = "nodes", 
        ...
        children = self.GRAPH_BUILDER.get_children(collection, parent_key, relation)
        if node_type:
    ...
    def find_module_calls(self) -> List[Dict[str, Any]]:
        ...
        for func_call in func_calls:
            ...
            # Find parent chunk
            inbound = self.GRAPH_BUILDER.get_inbound_edges("edges", func_call["_key"])
            chunk_key = None
            for edge in inbound:
    ...
    def _insert_nodes_and_edges(self, node_type: str, edge_relation: str, 
        ...
        for entity in entities:
            ...
            if not new_node_id:
                new_node_id = self.GRAPH_BUILDER.get_next_node_id()
            ...
            existing = self.GRAPH_BUILDER.get_node("knowledge_nodes", new_node_id)
            if not existing:
    ...
    #TODO: nest this insede insert local va r 
    #FIXME: Doesn't work for 'local z' 
    def find_local_var_declarations(self) -> List[Dict[str, Any]]:
        ...
        for chunk in chunks:
            ...
            for var_decl in var_decls:
                ...
                if not has_local: #TODO: able to extend this for global variables
                    continue
        ...
        for func_call in func_calls:
            ...
            for edge in inbound:
                ...
                if from_node and from_node.get("type") == "expression_list":
                    ...
                    assign_edges = self.GRAPH_BUILDER.get_inbound_edges("edges", from_key)
    ...
    def insert_required_modules(self):
        ...
        for module in required:
            ...
            existing = None
            kn_collection = self.GRAPH_BUILDER.get_collection("knowledge_nodes")
            ...
            if existing:
                ...
                module_id = self.GRAPH_BUILDER.get_next_node_id()
    ...
    def insert_functions(self):
        ...
        for chunk in chunks:
            ...
            kg_chunk = self.GRAPH_BUILDER.get_node("knowledge_nodes", chunk["_key"])
            if not kg_chunk:
                continue
            
            # Find module for this chunk
            module_node = None
            kg_children = self.GRAPH_BUILDER.get_children("knowledge_nodes", chunk["_key"], "defines")
    ...
    def _classify_functions(self):
        ...
        for node in kn.all():
            ...
            inbound = self.GRAPH_BUILDER.get_inbound_edges("knowledge_edges", node["_key"])
            declared_by_chunk = any(
                self.GRAPH_BUILDER.get_node("knowledge_nodes", e.get("_from", "").split("/")[-1])
                and self.GRAPH_BUILDER.get_node("knowledge_nodes", e.get("_from", "").split("/")[-1]).get("type") == "chunk"
    ...
    def insert_block_function_calls(self, block_id: str):
        ...
        for result in results:
            ...
            if existing:
                ...
                existing_call = self.GRAPH_BUILDER.get_node("knowledge_nodes", node["_key"])
        ...
        for block in kn.all():
            ...
            for child in block_children:
                if child.get("type") in self.control_statements:
                    ...
                    existing = self.GRAPH_BUILDER.get_node("knowledge_nodes", child["_key"])
                    if not existing:
                        results.append({
    ...
    def build_KG(self):
        ...
        self.get_start_node()
        self.insert_module_nodes() # TODO: Why are modules and function calls seperate if they do traverse the same structure
